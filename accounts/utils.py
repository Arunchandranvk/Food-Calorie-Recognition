...
def create_foodlist():
    food_list = ['carrot_cake', 'cheesecake', 'chicken_curry', 'chicken_wings', 'club_sandwich',
...
def predict_class(model, img):
    ...
    val = np.amax(pred)
    food_list = create_foodlist()  # Call create_foodlist to get the list of food items
...
def predict_volume(img):
    model = genai.GenerativeModel('gemini-pro-vision')
    input_prompt = "You are an expert in nutritionist where you need to see the which is the predicted food and tell only its total weight of the food ."
...
def process_image(image_file):
    ...
    img = img.resize((228, 228))  # Resize image to match model input size
    return img
